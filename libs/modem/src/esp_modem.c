...
static void uart_event_task_entry(void *param)
{
 ...
 //        Drive the event loop
        esp_event_loop_run(esp_dte->event_loop_hdl, pdMS_TO_TICKS(50));// Se comentar, ocorre timeout
    }
    vTaskDelete(NULL);
}
...
bool check_modem_conn_fail(void)
{
    return conn_fail;
}
