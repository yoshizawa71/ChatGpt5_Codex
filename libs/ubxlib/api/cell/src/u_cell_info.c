 ...
 * PUBLIC FUNCTIONS
 * -------------------------------------------------------------- */
// Get the raw CSQ value (0..31), as returned by AT+CSQ.
// Returns 99 se o valor for inválido ou não disponível.
/**
 * Get the raw CSQ value (0..31) if available, or a NB-IoT fallback estimate.
 * Returns 99 if the value is unavailable or on error.
 */
 
int32_t uCellInfoGetCsqRaw(uDeviceHandle_t cellHandle)
{
    const int32_t CSQ_UNKNOWN = 99;
    int32_t csq = CSQ_UNKNOWN;
    uCellPrivateInstance_t *pInstance;
    ...
    if (gUCellPrivateMutex != NULL) {
        ...
        if (pInstance != NULL) {
            ...
            // 1) tenta AT+CSQ diretamente
            uAtClientLock(atHandle);
            uAtClientCommandStart(atHandle, "AT+CSQ");
            uAtClientCommandStop(atHandle);
            uAtClientResponseStart(atHandle, "+CSQ:");
            csq = uAtClientReadInt(atHandle);      // <rssi>
            uAtClientSkipParameters(atHandle, 1);  // ignora <qual>
            uAtClientResponseStop(atHandle);
            err = uAtClientUnlock(atHandle);

            // se AT+CSQ deu certo e csq válido, retorna
            if ((err == 0) && (csq >= 0) && (csq <= 31)) {
                goto exit;
            }

            // 2) fallback para NB-IoT via AT+UCGED?
            uAtClientLock(atHandle);
            uAtClientCommandStart(atHandle, "AT+UCGED?");
            uAtClientCommandStop(atHandle);
            uAtClientResponseStart(atHandle, "+UCGED:");
            uAtClientSkipParameters(atHandle, 1);    // pula o "2"
            uAtClientResponseStart(atHandle, NULL);  // linha com RAT,MCC,MNC
            uAtClientSkipParameters(atHandle, 2);    // pula MCC,MNC
            uAtClientResponseStart(atHandle, NULL);  // linha com parâmetros detalhados
            uAtClientSkipParameters(atHandle, 5);    // EARFCN,Lband,ul_BW,dl_BW,TAC
            uAtClientSkipParameters(atHandle, 1);    // PCID
            int32_t rssiDbm = uAtClientReadInt(atHandle); // RSRP em dBm
            uAtClientResponseStop(atHandle);
            err = uAtClientUnlock(atHandle);

            // converte RSRP em CSQ estimado
            if (err == 0) {
                int32_t estimated = (rssiDbm + 113) / 2;
                // limita ao intervalo [0,31]
                if (estimated <   0) {
                    csq = 0;
                } else if (estimated > 31) {
                    csq = 31;
                } else {
                    csq = estimated;
                }
