                             ...
                             bool fromUrc)
{
    ...
    switch (status) {
        ...
        case U_CELL_NET_STATUS_REGISTERED_HOME:
            ...
            printf(">>>>>Registered on a roaming network (+CxREG: 1)<<<<<\n");
            if (printAllowed) {
                uPortLog("%d: RegH\n", rat3gpp);
            }
            break;
...
/*    CXREG_urc((uCellPrivateInstance_t *) pParameter,
              U_CELL_PRIVATE_NET_REG_TYPE_CGREG, 3  GSM/GPRS/EDGE );*/ //Atende ao GSM de outro modelo
//-------------------------------------------------------------------------------
//    Verificação do GSM para o modelo Sara R422
//-------------------------------------------------------------------------------
/*         status = CXREG_urc((uCellPrivateInstance_t *) pParameter,
              U_CELL_PRIVATE_NET_REG_TYPE_CGREG, 3);  */
           status = CXREG_urc((uCellPrivateInstance_t *) pParameter,
              U_CELL_PRIVATE_NET_REG_TYPE_CGREG, 0);
                           
              printf("<<<<<<<<<<<<< STATUS CREG>>>>>>>>>>> = %d\n",status);
}
        ...
        if ((pInstance != NULL) &&
            ...
            if (pBuffer != NULL) {
                ...
                for (size_t x = U_CELL_NET_SCAN_RETRIES + 1;
                   ...
                   // uAtClientCommandStart(atHandle, "AT+COPS=?");
                    uAtClientCommandStart(atHandle, "AT+COPS?");
                    uPortTaskBlock(1000);//Time to wait for net response
                    uAtClientCommandStop(atHandle);
