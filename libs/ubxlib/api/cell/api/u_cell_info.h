...
// em u_cell_info.h, logo após outros includes
typedef struct {
    int32_t rssiDbm;   // RSSI em dBm (GSM/2G)
    int32_t ber;       // Bit error rate (GSM/2G)
    int32_t rscpDbm;   // RSCP em dBm (UMTS, se suportado)
    int32_t ecn0Db;    // EC/NO em dB (UMTS, se suportado)
    int32_t rsrqDb;    // RSRQ em dB (LTE, Cat-M1/NB‑IoT)
    int32_t rsrpDbm;   // RSRP em dBm (LTE, Cat-M1/NB‑IoT)
    int32_t sinrDb;    // SINR em dB (LTE, Cat-M1/NB‑IoT — opcional)
} uCellInfoSignalQuality_t;
