...
#define U_PORT_UART_EVENT_MIN_TASK_STACK_SIZE_BYTES 2048
...
static void eventTask(void *pParam)
{
    ...
    const UBaseType_t STACK_WARN_THRESHOLD = 1024 / sizeof(StackType_t); // 1 KB de margem
    ...
    do {
                ...
                // ────────────── AQUI COMEÇA O PATCH ──────────────
        {
            ...
            if (highWater < STACK_WARN_THRESHOLD) {
                // imprime em bytes livres
                uPortLog("**WARN** eventTask stack low: %u bytes free (threshold %u)\n",
                         (unsigned)(highWater * sizeof(StackType_t)),
                         (unsigned)(STACK_WARN_THRESHOLD * sizeof(StackType_t)));
            }
        }
        // ────────────── AQUI TERMINA O PATCH ──────────────
...
int32_t uPortUartEventCallbackSet(int32_t handle,
                                  ...
                                  int32_t priority)
{
	  // DEBUG: imprima aqui para ver o valor real
    uPortLog("=====>>DEBUG: uPortUartEventCallbackSet() called with stackSizeBytes = %u\n",
             (unsigned) stackSizeBytes);
