...
static const char *TAG_UART = "LTE_UART_PWR";

static bool token_matches_andd_one(const char *token)
{
    if (token == NULL) {
        return false;
    }
                            ...
                            int32_t *upsv_out)
{
    ...
    int32_t error = uCellAtClientHandleGet(devHandle, &atHandle);
    if ((error != 0) || (atHandle == NULL)) {
        ESP_LOGE(TAG_UART, "AT handle indisponível para UPSV/DTR (err=%ld)", (long) error);
        return (int32_t) U_ERROR_COMMON_PLATFORM;
    }

    int32_t upsvMode = -1;
    bool upsvQueryOk = false;
    ...
    if (error != 0) {
        if (error == (int32_t) U_ERROR_COMMON_PROTOCOL_ERROR) {
            ESP_LOGW(TAG_UART, "AT+UPSV=4 não suportado (err=%ld)", (long) error);
            return (int32_t) U_ERROR_COMMON_NOT_SUPPORTED;
        }
        ESP_LOGE(TAG_UART, "Falha em AT+UPSV=4 (err=%ld)", (long) error);
        return error;
    }
    ...
    int32_t response = uAtClientResponseStart(atHandle, "+UPSV:");
    if (response == 0) {
        upsvMode = uAtClientReadInt(atHandle);
        upsvQueryOk = true;
    }
    ...
    if (error != 0) {
        if (error == (int32_t) U_ERROR_COMMON_PROTOCOL_ERROR) {
            ESP_LOGW(TAG_UART, "AT+UPSV? não suportado (err=%ld)", (long) error);
            return (int32_t) U_ERROR_COMMON_NOT_SUPPORTED;
        }
        ESP_LOGE(TAG_UART, "Falha em AT+UPSV? (err=%ld)", (long) error);
        return error;
    }
    ...
    if (upsvQueryOk) {
        ESP_LOGI(TAG_UART, "set UPSV=4 -> OK; UPSV?=%ld", (long) upsvMode);
    } else {
        ESP_LOGW(TAG_UART, "set UPSV=4 -> OK; UPSV? indisponível (ret=%ld)", (long) response);
    }

    int32_t finalStatus = (int32_t) U_ERROR_COMMON_SUCCESS;

    if (set_atd1) {
        bool anddConfirmed = false;
        uAtClientLock(atHandle);
        uAtClientCommandStart(atHandle, "AT&D=");
        uAtClientWriteInt(atHandle, 1);
        uAtClientCommandStopReadResponse(atHandle);
        error = uAtClientUnlock(atHandle);
        if (error != 0) {
            ESP_LOGW(TAG_UART, "Falha em AT&D=1 (err=%ld)", (long) error);
            finalStatus = error;
        } else {
            char token[64];
            uAtClientLock(atHandle);
            ...
            uAtClientCommandStop(atHandle);
            int32_t profileResp = uAtClientResponseStart(atHandle, "ACTIVE PROFILE:");
            if (profileResp == 0) {
                profileResp = uAtClientResponseStart(atHandle, NULL);
                if (profileResp == 0) {
                    while (true) {
                        int32_t len = uAtClientReadString(atHandle, token, sizeof(token), false);
                        if (len < 0) {
                            if (finalStatus == (int32_t) U_ERROR_COMMON_SUCCESS) {
                                finalStatus = (int32_t) U_ERROR_COMMON_DEVICE_ERROR;
                            }
                            break;
                        }
                        if (len == 0) {
                        ...
                        if (token_matches_andd_one(token)) {
                            anddConfirmed = true;
                        }
                    }
                } else if (finalStatus == (int32_t) U_ERROR_COMMON_SUCCESS) {
                    finalStatus = profileResp;
                }
            } else {
                ESP_LOGW(TAG_UART, "AT&V não retornou ACTIVE PROFILE (ret=%ld)", (long) profileResp);
                if (finalStatus == (int32_t) U_ERROR_COMMON_SUCCESS) {
                    finalStatus = profileResp;
                }
            }
            ...
            error = uAtClientUnlock(atHandle);
            if (error != 0) {
                ESP_LOGW(TAG_UART, "Falha ao ler AT&V (err=%ld)", (long) error);
                if (finalStatus == (int32_t) U_ERROR_COMMON_SUCCESS) {
                    finalStatus = error;
                }
            }
        }
        if (!anddConfirmed && (finalStatus == (int32_t) U_ERROR_COMMON_SUCCESS)) {
            finalStatus = (int32_t) U_ERROR_COMMON_NOT_FOUND;
        }
        ESP_LOGI(TAG_UART,
                 "set AT&D=1 -> %s; &V shows \"&D 1\" = %s",
                 (finalStatus == (int32_t) U_ERROR_COMMON_SUCCESS) ? "OK" : "FAIL",
                 anddConfirmed ? "YES" : "NO");
    }
...
int32_t lte_uart_psm_disable(uDeviceHandle_t devHandle)
{
    ...
    int32_t error = uCellAtClientHandleGet(devHandle, &atHandle);
    if ((error != 0) || (atHandle == NULL)) {
        ESP_LOGE(TAG_UART, "AT handle indisponível para desativar UPSV (err=%ld)", (long) error);
        return (int32_t) U_ERROR_COMMON_PLATFORM;
    }
    ...
    error = uAtClientUnlock(atHandle);
    if (error != 0) {
        ESP_LOGE(TAG_UART, "Falha em AT+UPSV=0 (err=%ld)", (long) error);
        return error;
    }
    ...
    error = uAtClientUnlock(atHandle);
    if (error != 0) {
        ESP_LOGE(TAG_UART, "Falha em AT+UPSV? após disable (err=%ld)", (long) error);
        return error;
    }

    if (upsvQueryOk) {
        ESP_LOGI(TAG_UART, "set UPSV=0 -> OK; UPSV?=%ld", (long) upsvMode);
    } else {
        ESP_LOGW(TAG_UART, "set UPSV=0 -> OK; UPSV? indisponível (ret=%ld)", (long) response);
    }
    ...
    error = uAtClientUnlock(atHandle);
    if (error != 0) {
        ESP_LOGW(TAG_UART, "Falha em AT&D=0 (err=%ld)", (long) error);
        return error;
    }
