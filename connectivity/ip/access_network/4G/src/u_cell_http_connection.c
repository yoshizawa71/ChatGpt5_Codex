...
//#define U_CELL_HTTP_TEST_FIRST_LINE_200_V1_0 "200"
...
//#define U_CELL_FILE_NAME "server_data.txt"
#define U_CELL_FILE_NAME  "server_data.json"
 ...
 * -------------------------------------------------------------- */
  ...
  bool http_request_delivery = false;
...
/*static const char gSendData[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
                                "0123456789!#$%&'()*+,-./:;<=>?@[\\]^_`{|}~";*/
//static const char gSendData[] = "Agora sim, Vai Corinthians!!!!";
...
static const char *TAG = "HTTP_Comunication";
...
static bool checkFile(uDeviceHandle_t devHandle, const char *pFileName,
                      const char *pExpectedFirstLine, bool printIt)
{
    ...
    fileSize = uCellFileSize(devHandle, pFileName);

    if (fileSize >= 0) {
        pFileContents = (char *) pUPortMalloc(fileSize);
        if (pFileContents != NULL) {
            if (uCellFileRead(devHandle, pFileName,
...
static void callback(uDeviceHandle_t devHandle, int32_t httpHandle,
                     ...
                     const char *pFileNameResponse, void *pCallbackParam)
{
    ...
    pCallbackData->contentsMismatch = !checkFile(devHandle,
                                                 pFileNameResponse,
...
static bool waitCheckHttpResponse(uint32_t timeoutSeconds,
                                  ...
                                  uDeviceHandle_t devHandle, int32_t httpHandle,
                                  uCellHttpRequest_t requestType,
                                  const char *pFileNameResponse)
{
    ...
    if (pCallbackData->called) {
        ...
        if (pCallbackData->contentsMismatch) {
            U_TEST_PRINT_LINE("contents of response were not as expected (expected 200 OK).");
            pCallbackData->error = true; // Marca como erro se o status não for 200 OK
        }
        isOk = true;
        ...
        if (pCallbackData->requestType != requestType) {
            U_TEST_PRINT_LINE("expected response type %s, got %s (%d).",
                              ...
                              printf(">>>>>>>>>Request type %d",requestType);
            isOk = false;
        }
        ...
        if (pFileNameResponse != NULL) {
			printf(">>>>>>>>>pFileNameResponse <<<<<<<<<< diferente de Null\n");
 ...
 */
//static void cellFileWrite(uDeviceHandle_t devHandle, char *data)
static void setup_file_params(uDeviceHandle_t devHandle, char *server_payload)
{
/*struct record_index_config rec_index = {0};
...
printf(">>>>>>CURSOR POSITION<<<<<<%d \n", cursor_position);*/
 //============================================   
    int32_t result;
    size_t y = 1;
    size_t length = strlen(server_payload);

    // Make sure the test file name has been deleted in case a
    // previous test was aborted half way
    
 //---------------------------------------
    uCellFileDelete(devHandle, U_CELL_FILE_NAME);

    for (size_t x = 0; x < y; x++) {
        if (x > 0) {
            U_TEST_PRINT_LINE("repeating with tag...");
            U_PORT_TEST_ASSERT(uCellFileSetTag(devHandle, "USER") == 0);
        } else {
            U_PORT_TEST_ASSERT(pUCellFileGetTag(devHandle) == NULL);
        }

        // Open file in write mode and write data into the file
        U_TEST_PRINT_LINE("writing data into file...");
        result = uCellFileWrite(devHandle, // Cellular Handle
                                U_CELL_FILE_NAME, // File name
                                server_payload, // Data to write into the file
                                length); // Data size
        U_TEST_PRINT_LINE("number of bytes written into the file = %d.", result);
        U_PORT_TEST_ASSERT(result == length);

        if (x > 0) {
            U_PORT_TEST_ASSERT(strcmp(pUCellFileGetTag(devHandle), "USER") == 0);
            U_PORT_TEST_ASSERT(uCellFileSetTag(devHandle, NULL) == 0);
        }
    }
}
 ...
 */
static void cellFileRead(uDeviceHandle_t devHandle)
{
...
/*    U_PORT_TEST_ASSERT(uCellTestPrivatePreamble(U_CFG_TEST_CELL_MODULE_TYPE,
                                                &gHandles, true) == 0);
    devHandle = gHandles.cellHandle;*/

    // Get the private module data as we need it for testing
/*    pModule = pUCellPrivateGetModule(devHandle);
...
/*    if (U_CELL_PRIVATE_HAS(pModule, U_CELL_PRIVATE_FEATURE_FILE_SYSTEM_TAG)) {
    ...
    for (size_t x = 0; x < y; x++) {
        ...
        length = uCellFileRead(devHandle, // Cellular Handle
                               U_CELL_FILE_NAME, // File name
...
//    int32_t y;
    ...
    char* url_server_domain = get_data_server_url();
    printf(">>>>>>URL HTTP = %s\n", url_server_domain);
    uint16_t url_server_port = get_data_server_port();
    printf(">>>>>> Server Port = %d\n", url_server_port);
    char* url_server_path = get_data_server_path();
...
//-----------------------------------------------------------
// Desativar URCs antes de iniciar operações HTTP
    uAtClientHandle_t atHandle;
    int32_t errorCode = uCellAtClientHandleGet(devHandle, &atHandle);
    if (errorCode == 0 && atHandle != NULL) {
        uPortLog("U_CELL: desativando URCs\n");
        ...
        uAtClientCommandStopReadResponse(atHandle);
        uAtClientCommandStart(atHandle, "AT+UHTTP=0");
        uAtClientCommandStopReadResponse(atHandle);
        uAtClientUnlock(atHandle);
        uPortTaskBlock(500); // Dar tempo ao modem
    } else {
...
//-----------------------------------------------------------        
        ...
        snprintf(urlBuffer, sizeof(urlBuffer), "%s:%d",
                 url_server_domain, url_server_port);
/*          snprintf(urlBuffer, sizeof(urlBuffer), "%s:%d",
                 U_HTTP_CLIENT_TEST_SERVER_DOMAIN_NAME, U_HTTP_CLIENT_TEST_SERVER_PORT);*/

        // Use the cellular module's IMEI as a "uniquifier" to avoid
        // collisions with other devices using the same HTTP test server
        U_PORT_TEST_ASSERT(uCellInfoGetImei(devHandle, imeiBuffer) == 0);
        imeiBuffer[sizeof(imeiBuffer) - 1] = 0;


        // Try using NULL parameters where they are not permitted in the open() call
      /*  U_PORT_TEST_ASSERT(uCellHttpOpen(devHandle, NULL, NULL, NULL,
                                         U_CELL_HTTP_TIMEOUT_SECONDS_MIN,
                                         callback, NULL) < 0);
        U_PORT_TEST_ASSERT(uCellHttpOpen(devHandle, urlBuffer, NULL, NULL,
                                         U_CELL_HTTP_TIMEOUT_SECONDS_MIN,
                                         NULL, NULL) < 0);
        U_PORT_TEST_ASSERT(uCellHttpOpen(devHandle, urlBuffer, NULL, "pw",
                                         U_CELL_HTTP_TIMEOUT_SECONDS_MIN,
                                         callback, NULL) < 0);*/
        ...
        httpHandle = uCellHttpOpen(devHandle, urlBuffer, NULL, NULL,
                                   U_CELL_HTTP_TIMEOUT_SECONDS_MIN,
                                   callback, (void *) &gCallbackData);
        U_PORT_TEST_ASSERT(httpHandle >= 0);
        U_PORT_TEST_ASSERT(!uCellHttpIsSecured(devHandle, httpHandle, NULL));
        ...
        if (pModule->moduleType == U_CELL_MODULE_TYPE_LEXI_R10) {
          ...
          //  gCallbackData.pExpectedFirstLine = U_CELL_HTTP_TEST_FIRST_LINE_200_V1_0;//Funciona com a versão antiga
              gCallbackData.pExpectedFirstLine = U_CELL_HTTP_TEST_FIRST_LINE_200_V1_1;
        }
          
 //       snprintf(pathBuffer, sizeof(pathBuffer), "/%s.html", imeiBuffer);
         snprintf(pathBuffer, sizeof(pathBuffer), url_server_path);
 //        snprintf(pathBuffer, sizeof(pathBuffer), "/data");
...
printf(">>>>>>CURSOR POSITION<<<<<<%d \n", cursor_position);
   ...
   setup_file_params(devHandle,server_payload);
...
/*          U_TEST_PRINT_LINE("HTTP POST file %s containing string \"%s\"...",
 ...
 /*       error =  uCellHttpRequest(devHandle, httpHandle,
                                            U_CELL_HTTP_REQUEST_POST,
                                            pathBuffer, NULL, gSendData,
                                            "application/txt");*/
                                                                  
                                            
/*        error = uCellHttpRequestFile(devHandle, httpHandle,
                                    U_CELL_HTTP_REQUEST_POST,
                                    pathBuffer, NULL,
                                    U_CELL_FILE_NAME,
                                    "application/txt");*/
        ...
        printf(">>>>>>>>>>Erro http request = %ld<<<<<<<<<<<\n", error);
                                                                                                                               
           
                                                 
        success =waitCheckHttpResponse(U_CELL_HTTP_TIMEOUT_SECONDS_MIN,
                                                 &gCallbackData, devHandle,
                                                 httpHandle,
                                                 U_CELL_HTTP_REQUEST_POST, NULL);                                                 
         uPortTaskBlock(300); 
                                             
          printf(">>>>>>>>>>Erro Wait Check Http Response = %s<<<<<<<<<<<\n", success ? "200 Ok recebido" : "Sem Resposta");
          
    // Reativar URCs após a requisição POST
    errorCode = uCellAtClientHandleGet(devHandle, &atHandle);
    if (errorCode == 0 && atHandle != NULL) {
        uPortLog("U_CELL: reativando URCs\n");
        uAtClientLock(atHandle);
        ...
        uAtClientCommandStopReadResponse(atHandle);
        uAtClientCommandStart(atHandle, "AT+UHTTP=2");
        uAtClientCommandStopReadResponse(atHandle);
        uAtClientUnlock(atHandle);
        uPortTaskBlock(500); // Dar tempo ao modem
    } else {
...
/*        // GET it
   ...
   //     cellFileRead(devHandle);
        ...
        printf("################## já saiu do response \n");
        uCellHttpGetLastErrorCode(devHandle, httpHandle);

        // Close the HTTP instance once more
        uCellHttpClose(devHandle, httpHandle);

        // Delete our data file for neatness
        uCellFileDelete(devHandle, U_CELL_HTTP_TEST_DATA_FILE_NAME);
    } else {
