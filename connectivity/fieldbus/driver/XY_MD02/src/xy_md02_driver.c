...
{
	ESP_LOGI("XY_MD02", "try_fc: Tentando addr=%d fc=0x%02X reg=0x%04X", addr, fc, reg);
    esp_err_t err;
/*    if (fc == 0x04) {
    ...
    if (err == ESP_OK) {
        ESP_LOGI("XY_MD02", "try_fc sucesso: addr=%d fc=0x%02X reg=0x%04X value=0x%04X", addr, fc, reg, *out);
        return true;
    } else {
